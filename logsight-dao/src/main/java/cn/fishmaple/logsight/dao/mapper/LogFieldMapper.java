...
public interface LogFieldMapper {
    @Select("SELECT id ,status,`file_count` fileCount,`size`,`path` FROM `log_field` WHERE `id` = #{id}" )
    ...
    @Update("UPDATE `log_field` SET create_time = #{createTime},path = #{path},`status` =#{status},file_count = null,size = null WHERE `id` = #{id}")
    ...
    @Select("SELECT `id`,`path`,`timeline`,`status`,`create_time` createTime,`file_count` fileCount,`size` FROM `log_field` LIMIT #{start},#{count}")
    public List<LogFieldDTO> selectAPage(@Param("start") int start, @Param("count")int count);
    @Select("SELECT `id`,`path`,`timeline`,`status`,`create_time` createTime,`file_count` fileCount,`size`  FROM `log_field` " +
